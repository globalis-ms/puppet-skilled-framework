...
class Builder
{
     ...
     * @param  mixed  $id
     * @param  array  $columns
     * @return \Globalis\PuppetSkilled\Database\Magic\Model|\Globalis\PuppetSkilled\Database\Magic\Collection|static[]|static|null
     */
     ...
     *
     * @param  array  $columns
     * @return \Globalis\PuppetSkilled\Database\Magic\Model|static|null
     */
    ...
    {
        $dots = (mb_strpos($name, '.') !== false);

        return $dots && (substr($name, 0, strlen($relation.'.')) ===  $relation.'.');
    }
     ...
     *
     * @param  \Globalis\PuppetSkilled\Database\Magic\Builder  $relation
     * @return \Globalis\PuppetSkilled\Database\Magic\Builder|static
     */
    ...
    protected function parseWithRelations(array $relations)
    {
        ...
        foreach ($relations as $name => $constraints) {
            ...
            if (is_numeric($name)) {
                if (mb_strpos($constraints, ':') !== false) {
    ...
    protected function addNestedWhereSlice(QueryBuilder $query, $wheres, $offset, $length = null)
    {
        ...
        if (in_array('or', $whereBooleans)) {
            $query->wheres[] = $this->nestWhereSlice($whereSlice, $whereBooleans[0]);
        } else {
